...
describe("Atomic update + audit, one domain per RPC", () => {
  ...
  it("Markets: updates ingestion, publication, and social access policy atomically", async () => {
    const at = await currentUpdatedAt();
    const values = { marketIngestionEnabled: false, marketIngestionMinBookmakerCount: 4, postPublicationEnabled: false, postPublicationRequiresActiveMarket: false, socialPredictionEnabled: true, feedCompletedGameRetentionHours: 24 };
...
describe("Internal role check (R13 defense-in-depth)", () => {
  ...
  const CALLS: Array<{ rpc: string; args: Record<string, unknown> }> = [
    ...
    { rpc: "update_market_settings", args: { p_market_ingestion_enabled: true, p_market_ingestion_min_bookmaker_count: 2, p_post_publication_enabled: true, p_post_publication_requires_active_market: true, p_social_prediction_enabled: false, p_feed_completed_game_retention_hours: 24 } },
