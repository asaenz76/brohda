...
export default async function FollowersPage({
  ...
  const { data: rows } = await supabase.rpc("get_followers", {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-lg font-bold text-text-primary">
        {profile.id === currentUser.id ? "Your followers" : `${profile.display_name}'s followers`}
      </h1>

      {entries.length === 0 ? (
        <EmptyFeedState icon={Users} title="No followers yet" description="Nobody's following yet." />
      ) : (
